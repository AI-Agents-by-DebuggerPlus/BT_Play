...
object ChatMessageFilter {
    fun shouldShowInChat(message: ChatMessage, settings: AppSettings): Boolean {
        ...
        if (com.taskertowpf.androidchatcopyv1.LogMessageFormat.isLogContent(message.content)) {
            return false
        }
        if (com.taskertowpf.androidchatcopyv1.hrt.HrtProtocol.isRemoteTerminal(message)
            || com.taskertowpf.androidchatcopyv1.hrt.HrtProtocol.isHwtContent(message.content)
        ) {
            return false
        }

        val inbox = settings.incomingRecipientName.trim().ifBlank { "Android" }
        val mine = settings.senderName.trim().ifBlank { "AndroidChatCopyV1" }
