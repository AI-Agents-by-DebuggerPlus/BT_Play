    ...
    fun notifyButton(buttonLabel: String, source: String = "native") {
        ...
        scope.launch {
            ...
            if (isBtPlayLabel(label)) {
                ...
                if (btPlayTestIsolation) {
                    ...
                    if (handler != null) {
                        app.localLogRepository.logLocal(
                            "Headset",
                            "$kind: $label via $source → isolated handler",
                        )
                        ...
                        app.localLogRepository.logLocal(
                            "Headset",
                            "$kind: $label via $source → isolation (только счётчик)",
                        )
                ...
                app.localLogRepository.logLocal(
                    "Headset",
                    "$kind: $label via $source → STT / pause / next",
                )
            ...
            if (result.isFailure) {
                ...
                app.localLogRepository.logLocal("Headset", "$kind: $label via $source → $recipient")
            }
        }
    }
