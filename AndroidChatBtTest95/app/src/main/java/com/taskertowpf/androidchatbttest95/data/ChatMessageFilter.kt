...
object ChatMessageFilter {
    fun shouldShowInChat(message: ChatMessage, settings: AppSettings): Boolean {
        ...
        if (com.taskertowpf.androidchatbttest95.LogMessageFormat.isLogContent(message.content)) {
            return false
        }
        if (com.taskertowpf.androidchatbttest95.hrt.HrtProtocol.isRemoteTerminal(message)
            || com.taskertowpf.androidchatbttest95.hrt.HrtProtocol.isHwtContent(message.content)
        ) {
            return false
        }

        val inbox = settings.incomingRecipientName.trim().ifBlank { "Android" }
        val mine = settings.senderName.trim().ifBlank { "AndroidChatBtTest95" }
