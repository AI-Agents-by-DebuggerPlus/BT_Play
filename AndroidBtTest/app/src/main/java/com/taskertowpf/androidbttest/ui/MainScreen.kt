package com.taskertowpf.androidbttest.ui
...
import com.taskertowpf.androidbttest.AppBuildInfo
import com.taskertowpf.androidbttest.MainViewModel
import com.taskertowpf.androidbttest.bluetooth.BluetoothDeviceRow
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                title = { Text("AndroidBtTest ${AppBuildInfo.versionLabel}") },
                actions = {
        ...
        Column(
            ...
            when (state.selectedTab) {
                ...
                2 -> InterceptMonitorSection(
                    activeSessionOwners = state.activeSessionOwners,
                    ownDeviceLabel = state.snapshot?.activeSummary.orEmpty().ifBlank { "—" },
                    isOwnSessionTop = state.activeSessionOwners.firstOrNull() ==
                        "com.taskertowpf.androidbttest",
                    onOpenNotificationAccessSettings = viewModel::openNotificationAccessSettings,
                )
                3 -> LogsSection(
...
@Composable
private fun InterceptMonitorSection(
    activeSessionOwners: List<String>,
    ownDeviceLabel: String,
    isOwnSessionTop: Boolean,
    ...
    Column(
        ...
        Text(
            text = "Кто сейчас перехватывает кнопку (в реальном времени)",
            style = MaterialTheme.typography.titleMedium,
        )
        if (activeSessionOwners.isEmpty()) {
            Text(
                text = "Нет данных — включите Notification Access",
            ...
            activeSessionOwners.forEachIndexed { index, pkg ->
                val isTop = index == 0
                val isSelf = pkg == "com.taskertowpf.androidbttest"
                Text(
                    text = "${index + 1}. $pkg" + if (isTop) "  ← получает кнопку сейчас" else "",
                    color = when {
                        isTop && isSelf -> MaterialTheme.colorScheme.primary
                        isTop -> MaterialTheme.colorScheme.error
        ...
        Text(
            text = if (isOwnSessionTop) {
                "✓ AndroidBtTest перехватило: \"$ownDeviceLabel\""
            } else {
                "✗ AndroidBtTest НЕ является текущим получателем кнопки"
            },
            style = MaterialTheme.typography.bodyLarge,
            color = if (isOwnSessionTop) {
...
private fun BtPlayTestSection(
    ...
    Column(
        ...
        Text(
            text = "Сначала включите «доступ к уведомлениям» (кнопка ниже). Затем «Диагностика → логи» " +
                "и смотрите ActiveSessions — первый пакет получает кнопку. Если не " +
                "com.taskertowpf.androidbttest — закройте его (Force Stop) и снова Play. " +
...
private fun LogsSection(
    ...
    Column(Modifier.fillMaxSize()) {
        ...
        LazyColumn(
            ...
            items(entries, key = { "${it.timestampMillis}-${it.message}" }) { entry ->
                LogLine(entry)
            }
            item { Spacer(Modifier.height(8.dp)) }
        }
...
private fun SettingsScreen(viewModel: MainViewModel) {
    ...
    Scaffold(
        ...
        Column(
            ...
            Text("sender_name = AndroidBtTest, content = [LOG:{category}] …")
            OutlinedTextField(
